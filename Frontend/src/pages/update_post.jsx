...
function UpdatePost () {
    ...
    useEffect(() => {
        ...
        // When post.description is available, set value
        if (editorInstance.current && post?.description) {
            editorInstance.current.value(post.description);
        }
    
    }, [post?.description]); // re-run when post.description updates
        ...
        <div className="bg-white rounded-xl shadow-md overflow-hidden p-6">
            <form onSubmit={handleSubmit}>
                <div className="mb-6">
                    ...
                    <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} name="title" id="title" className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent" placeholder="Enter post title" required />
                </div>

                <div className="mb-6">
                    <label htmlFor="subheading" className="block text-gray-700 font-medium mb-2">Subheading</label>
                    <input type="text" value={subheading} onChange={(e) => setSubheading(e.target.value)} name='subheading' id="subheading" className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent" placeholder="Enter a brief subheading" required />
                </div>
                ...
                <div className="mb-6">
                    ...
                    {previewUrl && (
                        <div className="mt-4">
                            ...
                            />
                            <p className="text-center text-sm text-gray-600 mt-2"></p>
                        </div>
                    )}
                </div>
